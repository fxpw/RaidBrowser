...
function E.Core:FindILVL(message)
    ...
    for _, pattern in pairs(E.Patterns.ILVL) do
        ...
        if ilvl and ilvl > 10 and ilvl < 79 then
            tableFR["ilvl"] = ilvl;
            message = string.gsub(message, pattern, '')
...
local function ChatParserFunc(self, event, message, sender, language, _, _, _, _, _, _, _, _, guid)
    ...
    if raidInfo.find and roles.find then
        ...
        -- Safe instance name handling
        local numRaidsWhithoutCD = 0
        if raidInfo.instanceName then
            numRaidsWhithoutCD = #raidInfo.instanceName
        ...
        -- Safe configOrder handling
        local shouldAdd = true
        if raidInfo.configOrder and E.db["RaidBlackList" .. raidInfo.configOrder] then
            shouldAdd = false
        end

        if E.db.HideRaidsWithCD then
            if numRaidsWhithoutCD > 0 and shouldAdd then
